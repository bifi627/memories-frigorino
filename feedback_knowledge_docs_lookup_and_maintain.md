...
The same verify-first habit applies to tracker items: when picking up an `IDEAS.md` / `TECH_DEBT.md` entry, fact-check its `file:line` and field/predicate claims against current code *before* writing anything, and state what has drifted (or confirm it's exact) up front — "lets work on this item, first fact check if this still holds." Tracker entries go stale like docs do.
