...
**Who does the browser verify (2026-06-22):** the manual browser verification is usually done by the **user**, not the agent — they often prefer to quick-test the UI themselves rather than have the agent spin up the dev stack. Clarify the gate up front and hand off (route + what to check) rather than reflexively running `dev-up`. See [[feedback_clarify_ui_gate_upfront]].
